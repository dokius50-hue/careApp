...
test('income and expense appear in lists with euro formatting and export opens overlay', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /save/i }).first().click();
  await page.waitForTimeout(1500);
  await expect(page.getByTestId('income-entry').filter({ hasText: incomeName }).first()).toBeVisible({ timeout: 15000 });
  await expect(page.getByTestId('income-entry').filter({ hasText: '€10.50' }).first()).toBeVisible();
  await expect(page.getByTestId('expense-entry').filter({ hasText: expenseName }).first()).toBeVisible({ timeout: 15000 });
  await expect(page.getByTestId('expense-entry').filter({ hasText: '€5.25' }).first()).toBeVisible();
